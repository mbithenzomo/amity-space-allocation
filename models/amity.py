...
class Amity():
	...
	def create_room(self, args):
		"""Create new room(s)"""
		print spacer
		initial_count, final_count = len(self.rooms), 0
		new_rooms = []
		for room in args["<room_name>"]:
			...
			for r in self.rooms:
				if r.name == new_room.name:
					print "A room you tried to create already exists! Please try again."
					return
		...
		for new_room_ in new_rooms:
			print "Name: " + ''.join(new_room_.name) + " | Type: " + new_room_.room_type
		print spacer
